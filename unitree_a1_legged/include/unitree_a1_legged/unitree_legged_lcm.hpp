...
{
public:
    explicit UnitreeLegged() : safe(LeggedType::A1), udp(LOWLEVEL)
    {
        udp.InitCmdData(cmd);
    }
    void UDPRecv();
...
void UnitreeLegged::UDPRecv()
{
    udp.Recv();
}

void UnitreeLegged::UDPSend()
{
    udp.Send();
}
...
void UnitreeLegged::RobotControl()
{
    motiontime++;
    udp.GetRecv(state);

    // pub_low->publish(stateToRos(state));
